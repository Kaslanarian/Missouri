...
DEFINE_string(camera_resolution,        "-1x-1",        "Set the camera resolution (either `--camera` or `--flir_camera`). `-1x-1` will use the"
                                                        ...
                                                        " `--flir_camera`");
DEFINE_string(video,                    "data/data (9).mp4","Use a video file instead of the camera. Use `examples/media/video.avi` for our default"
                                                        " example video.");
...
DEFINE_uint64(frame_step,               3,              "Step or gap between processed frames. E.g., `--frame_step 5` would read and process frames"
                                                        " 0, 5, 10, etc..");
...
DEFINE_bool(part_candidates,            true,          "Also enable `write_json` in order to save this information. If true, it will fill the"
...
DEFINE_string(write_images_format,      "jpg",          "File extension and format for `write_images`, e.g., png, jpg or bmp. Check the OpenCV"
...
DEFINE_string(write_json,               "example",             "Directory to write OpenPose output in JSON format. It includes body, hand, and face pose"
...
DEFINE_int32(write_coco_json_variants,  3,              "Add 1 for body, add 2 for foot, 4 for face, and/or 8 for hands. Use 0 to use all the"
